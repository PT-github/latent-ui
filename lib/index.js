  ...
  return {
    captureAnimationState: function captureAnimationState() {
      ...
      children.forEach(function (child) {
        ...
        var fromRect = _objectSpread$1({}, animationStates[animationStates.length - 1].rect); // If animating: compensate for current animation
  ...
  pluginEvent: function pluginEvent(eventName, sortable, evt) {
    ...
    plugins.forEach(function (plugin) {
      ...
      if (sortable[plugin.pluginName][eventNameGlobal]) {
        sortable[plugin.pluginName][eventNameGlobal](_objectSpread$1({
          sortable: sortable
        }, evt));
      ...
      if (sortable.options[plugin.pluginName] && sortable[plugin.pluginName][eventName]) {
        sortable[plugin.pluginName][eventName](_objectSpread$1({
          sortable: sortable
        }, evt));
      }
    });
...
function dispatchEvent(_ref) {
  ...
  var allEventProperties = _objectSpread$1({}, extraEventProperties, PluginManager.getEventProperties(name, sortable));
...
var pluginEvent = function pluginEvent(eventName, sortable) {
  ...
  PluginManager.pluginEvent.bind(Sortable)(eventName, sortable, _objectSpread$1({
    dragEl: dragEl,
    parentEl: parentEl,
    ghostEl: ghostEl,
    rootEl: rootEl,
...
/** @lends Sortable.prototype */
{
  ...
  evt) {
    ...
    function dragOverEvent(name, extra) {
      pluginEvent(name, _this, _objectSpread$1({
        evt: evt,
        isOwner: isOwner,
...
Sortable.mount = function () {
  ...
  plugins.forEach(function (plugin) {
    ...
    if (plugin.utils) Sortable.utils = _objectSpread$1({}, Sortable.utils, plugin.utils);
...
var table = {
  ...
  render: function render() {
    ...
    return h("div", {
      ...
      "props": _objectSpread$2({}, this.$attrs),
      ...
      return h(LTableColumn, {
        ...
        "scopedSlots": _objectSpread$2({}, _this.$scopedSlots)
      });
  ...
  watch: {
    columns: {
      handler: function handler(v) {
        console.log(v, 'columns发生了变化'); // console.log(this.$refs.table.store.updateColumns())
      },
      deep: true
    }
  },
      ...
      this.sortable = Sortable.create(wrapperTr, {
        ...
        onEnd: function onEnd(evt) {
          if (evt.oldIndex !== evt.newIndex) {
            var oldItem = _this2.$refs.table.store.states._columns[evt.oldIndex];

            _this2.$refs.table.store.commit('removeColumn', oldItem);

            _this2.$refs.table.store.commit('insertColumn', oldItem, evt.newIndex);
          }
        }
      });
    }
  ...
  methods: {
    ...
    loadData: function loadData() {
      ...
      if (this.dataSource instanceof Array) {
        ...
        var queryParams = _objectSpread$2((_objectSpread2 = {}, defineProperty(_objectSpread2, this.attrProps && this.attrProps.pageNo ? this.attrProps.pageNo : 'pageNo', this.localPagination.pageNo), defineProperty(_objectSpread2, this.attrProps && this.attrProps.pageSize ? this.attrProps.pageSize : 'pageSize', this.localPagination.pageSize), _objectSpread2), this.queryParams);

        var result = this.dataSource(queryParams);

        if (result && result.then) {
          result.then(function (r) {
            var _this3$tableData;

            _this3.asyncLocalPagination(r); // 为防止删除数据后导致页面当前页面数据长度为 0 ,自动翻页到上一页


            (_this3$tableData = _this3.tableData).push.apply(_this3$tableData, toConsumableArray(r.list));

            _this3.$emit('load-finished');
    ...
    handleTableHeight: function handleTableHeight() {
      ...
      this.timer = setTimeout(function () {
        var height = window.getComputedStyle(_this4.parentNode).height; // 外部容器高度

        if (_this4.showPage) {
          try {
            height = Number(height.substring(0, height.length - 2)) - Number(_this4.paginationHeight.substring(0, _this4.paginationHeight.length - 2)) + 'px';
          } catch (error) {
            console.error('table组件计算pagination高度发生错误', error);
          }
        }

        _this4.tHeight = height;
      }, 0);
    },
...
var querytable = {
render: function(){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{staticClass:"l-querytable"},[_c('l-queryform',_vm._b({on:{"input":_vm.handleInput,"search":_vm.handleSearch}},'l-queryform',_vm.$attrs,false)),_vm._v(" "),_c('div',{staticClass:"l-querytable_table"},[_c('l-table',_vm._b({ref:"table",attrs:{"queryParams":_vm.$attrs.value}},'l-table',_vm.$attrs,false))],1)],1)},
staticRenderFns: [],
  name: 'LQuerytable',
  props: {},
  data () {
...
/*
 * @Author: PT
 * @Date: 2020-05-08 17:50:00
 * @LastEditors: PT
 * @LastEditTime: 2020-05-08 17:50:24
 * @Description: file content
 */
 ...
 * @Description: 
 */
var components = [button, table, icon, form, queryform, querytable];
