...
var LTableColumn = {
render: function(){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('el-table-column',_vm._b({scopedSlots:_vm._u([{key:"default",fn:function(scope){return [(_vm.item.slotbool)?[_vm._t(_vm.item.value,null,{"row":scope.row,"column":scope.column,"index":scope.$index})]:[_c('span',[_vm._v(_vm._s(scope.row[_vm.item.value]))])]]}}],null,true)},'el-table-column',_vm.$attrs,false),[_vm._v(" "),(_vm.item.children && _vm.item.children.length > 0)?_vm._l((_vm.item.children),function(column,idx){return _c('l-table-column',{key:'column_' + idx + '_' + _vm.item.value,attrs:{"item":column,"type":_vm.item.type,"index":column.index,"column-key":column.columnKey,"label":column.label,"prop":column.prop,"width":column.width,"min-width":column.minWidth,"fixed":column.fixed,"render-header":column.renderHeader,"sortable":column.sortable,"sort-method":column.sortMethod,"sort-by":column.sortBy,"sort-orders":column.sortOrders,"resizable":column.resizable !== false,"formatter":column.formatter,"show-overflow-tooltip":column.showOverflowTooltip,"align":column.align,"header-align":column.headerAlign,"class-name":column.className,"label-class-name":column.labelClassName,"selectable":column.selectable,"reserve-selection":column.reserveSelection,"filters":column.filters,"filter-placement":column.filterPlacement,"filter-multiple":column.filterMultiple,"filter-method":column.filterMethod,"filtered-value":column.filteredValue}})}):_vm._e()],2)},
staticRenderFns: [],
  name: 'LTableColumn',
  props: {
    item: {
